...
class Ad extends Base
{
    ...
    public function index(){
        ...
        $key = input('key');
        $map = [];
        $map['closed'] =config('default_status');
        if($key&&$key!=="")
        {
            $map['title'] = ['like',"%" . $key . "%"];          
        }             
        ...
        foreach ($lists as $k => $v) {
               $lists[$k]['start_date'] = $v['start_date'] ? $v['start_date'] : '';
               $lists[$k]['end_date'] = $v['end_date'] ? $v['end_date'] : '';
           }   
    ...
    public function add_ad()
    {
        if(request()->isAjax()){
            ...
            $param = input('post.');
            $param['closed'] = 0;
            $param['start_date'] = $param['start_date'] ? $param['start_date'] : NULL;
            $param['end_date'] = $param['end_date'] ? $param['end_date'] : NULL;
            $ad = new AdModel();
    ...
    public function ad_state()
    {
        ...
        if($status==1)
        {
            $flag = Db::name('ad')->where(array('id'=>$id))->setField(['status'=>config('close_status')]);
            return json(['code' => 1, 'data' => $flag['data'], 'msg' => '已禁止']);
        }
        else
        {
            $flag = Db::name('ad')->where(array('id'=>$id))->setField(['status'=>config('normal_status')]);
    ...
    public function position_state()
    {
        ...
        if($status==1)
        {
            $flag = Db::name('ad_position')->where(array('id'=>$id))->setField(['status'=>config('close_status')]);
            return json(['code' => 1, 'data' => $flag['data'], 'msg' => '已禁止']);
        }
        else
        {
            $flag = Db::name('ad_position')->where(array('id'=>$id))->setField(['status'=>config('normal_status')]);
            return json(['code' => 0, 'data' => $flag['data'], 'msg' => '已开启']);
        }  
    }  
